...
## [渲染结果缓存 v-once](https://cn.vuejs.org/v2/guide/components.html#对低开销的静态组件使用-v-once)
...
web端渲染
1. SEO 很多爬虫是不认的。
2. 速度，首次打开速度,各种加载 速度很慢。
3. 服务器成本

如果没有这两点要求就不用考虑服务端渲染，会增加服务器压力（占用大量cpu,还得考虑负载），这个成本还是交给用户吧（可以考虑首页使用服务器渲染）
...
>如果你倾向于使用提供了平滑开箱即用体验的更高层次解决方案，你应该去尝试使用 Nuxt.js。它建立在同等的 Vue 技术栈之上，但抽象出很多模板，并提供了一些额外的功能，例如静态站点生成。但是，如果你需要更直接地控制应用程序的结构，Nuxt.js 并不适合这种使用场景。